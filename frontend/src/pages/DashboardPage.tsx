...
import { pagesLabel, readingProgressLabel } from "@/lib/bookProgress";
    ...
    <div className="grid gap-7">
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
        {current ? (
          ...
          <Card padding="lg" className="grid min-h-72 place-items-center">
            <EmptyState
              ...
              action={
                <Link className="inline-flex rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-bg hover:bg-accent-dim" to="/app/library">
                  Open library
                </Link>
              }
            />
        ...
        <div className="grid content-start gap-3 sm:grid-cols-2 xl:grid-cols-1">
          <StatCard label="current streak" value="Not tracked" hint="No reading activity log exists yet" />
          <StatCard label="pages today" value="Not tracked" hint="Requires daily progress events" />
          <StatCard
            label="annual goal"
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/75 p-4">
          ...
          >
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-semibold uppercase text-accent">Update progress</p>
    ...
    <Card padding="lg" className="grid gap-5 overflow-hidden border-accent/20 bg-surface shadow-glow md:grid-cols-[160px_minmax(0,1fr)]">
      ...
      <div className="grid min-w-0 content-between gap-5">
        <div className="min-w-0">
          <p className="text-xs font-semibold uppercase text-accent">Currently reading</p>
        ...
        <div className="grid gap-3">
          <div className="flex items-end justify-between gap-3">
            <div>
              <p className="font-mono text-2xl font-semibold text-text">{Math.round(book.progress_pct)}%</p>
              <p className="text-sm text-text-muted">{pagesLabel(book)}</p>
            </div>
            <p className="text-right text-sm text-text-muted">{readingProgressLabel(book)}</p>
          </div>
          <ProgressBar value={book.progress_pct} />
          <div className="flex flex-wrap gap-2">
            <Link className="inline-flex rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-bg hover:bg-accent-dim" to={`/app/book/${encodeURIComponent(book.id)}`}>
              Continue
            </Link>
  ...
  return (
    <Card className="grid gap-3">
      <div className="grid grid-cols-[72px_minmax(0,1fr)] gap-3">
        ...
        <div className="min-w-0">
          <p className="text-xs font-semibold text-accent">{recommendationMatchPercent(item.score)}% match</p>
