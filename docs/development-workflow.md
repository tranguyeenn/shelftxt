...
Day-to-day guide for working on ShelfTxt — from first clone through pull request. For install commands and env vars, see [development.md](development.md). For repository layout, see [architecture/system-overview.md](architecture/system-overview.md).
...
|-----------------|---------|
| API paths or payloads | [api.md](api.md), [system-design/api-design.md](system-design/api-design.md) |
...
| Non-obvious trade-off | [decisions.md](decisions.md) or a [devlog](devlogs/) entry |
| Refactor worth remembering | [DEVLOG.md](../DEVLOG.md) + `docs/devlogs/YYYY-MM-DD-*.md` |
...
3. Update [api.md](api.md) and frontend types in `frontend/src/lib/types.ts` if needed.
4. Add or update API tests in `tests/test_api.py`.

### Run the batch ingest pipeline locally

See [pipeline.md](pipeline.md). The live UI import path (`POST /books/import`) is separate from `backend/ingest/`.
...
| Architecture questions | [architecture/system-overview.md](architecture/system-overview.md) |
